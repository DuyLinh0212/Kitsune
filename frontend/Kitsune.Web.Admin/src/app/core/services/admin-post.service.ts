...
export class AdminPostService {
  getAllPosts(): Observable<AdminPostDto[]> {
    return from(
      supabase
        .from('Posts')
        .select(`
          Id, UserId, Title, Content, ImageUrls, CreatedAt,
          Users:UserId(Username, FullName),
  ...
  getPostComments(postId: number): Observable<AdminCommentDto[]> {
    return from(
      ...
      map(({ data, error }) => {
        if (error) throw error;
        return (data ?? []).map((r) => {
          const user = r['Users'] as { Username: string; FullName: string | null } | null;
          return {
            id: r['Id'] as number,
  ...
  deleteComment(commentId: number): Observable<void> {
    return from(supabase.from('Comments').delete().eq('Id', commentId)).pipe(
      map(({ error }) => {
        if (error) throw error;
      })
    );
