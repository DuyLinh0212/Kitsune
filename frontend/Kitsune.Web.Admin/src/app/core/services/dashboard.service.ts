...
export interface DashboardStats {
  ...
  totalStructures: number; totalFolders: number; newUsersToday: number;
...
export class DashboardService {
  ...
  private async fetchStats(): Promise<DashboardStats> {
    ...
    const [
      ...
      totalStructuresRes, totalFoldersRes, newUsersTodayRes,
      usersLastPeriodRes, vocabLastPeriodRes, languageBreakdownRes,
      recentUsersRes, recentActivityRes, chartUsersRes, chartVocabRes
    ] = await Promise.all([
      supabase.from('Users').select('*', { count: 'exact', head: true }),
      supabase.from('Vocabularies').select('*', { count: 'exact', head: true }),
      supabase.from('Kanji').select('*', { count: 'exact', head: true }),
      supabase.from('Radicals').select('*', { count: 'exact', head: true }),
      supabase.from('KanjiStructures').select('*', { count: 'exact', head: true }),
      supabase.from('VocabularyFolders').select('*', { count: 'exact', head: true }),
      ...
      supabase.from('Vocabularies').select('LanguageId, Languages:LanguageId(LanguageName, LanguageCode)', { count: 'exact' }),
      supabase.from('Users').select('Username, Email, FullName, AvatarUrl, CreatedAt').order('CreatedAt', { ascending: false }).limit(10),
      supabase.from('UserActivityLogs').select('CreatedAt, Users:UserId(Username), Action, Description').order('CreatedAt', { ascending: false }).limit(20),
    ...
    const totalStructures = totalStructuresRes.count ?? 0;
    ...
    for (const row of languageBreakdownRes.data ?? []) {
      const lang = row['Languages'] as { LanguageName: string; LanguageCode: string } | null;
      if (!lang) continue;
    ...
    const recentActivity: ActivityLogDto[] = (recentActivityRes.data ?? []).map((a) => {
      const userRow = a['Users'] as { Username: string } | null;
      return { timestamp: a['CreatedAt'] as string, username: userRow?.Username ?? 'unknown', action: a['Action'] as string, description: (a['Description'] as string | null) ?? null };
    });
    ...
    return { totalUsers, totalVocabulary, totalKanji, totalRadicals, totalStructures, totalFolders, newUsersToday, userGrowthPercent, vocabularyGrowthPercent, languageBreakdown, recentUsers, recentActivity, chart: { labels, newUsersSeries, newVocabularySeries } };
  }
}
