...
export class AuthService {
	...
	login(payload: LoginRequest): Observable<AuthResponse> {
		// Nếu không chứa @ → là username → tra email từ Users table trước
	...
	updateProfile(patch: { fullName?: string | null; username?: string }): Observable<UserProfile> {
		return from(this.getCurrentUserId()).pipe(
			switchMap((userId) => {
				...
				return from(
					supabase.from('Users').update(update).eq('Id', userId).select('Id, Username, Email, FullName, AvatarUrl, IsVerified, CreatedAt, User_Role(Role(RoleName))').single()
				);
			}),
			map(({ data, error }) => {
				if (error) throw error;
	...
	// Lookup bằng Email (AuthUserId không có trong schema)
	// Nếu chưa có row → tự tạo profile cho user Supabase Auth
	...
	private async createUserProfile(supaUser: User): Promise<UserProfile | null> {
		...
		// Tạo username từ phần trước @ của email, cắt tối đa 40 ký tự
		const base = (meta['username'] as string | undefined)
			?? email.split('@')[0].replace(/[^a-zA-Z0-9_]/g, '_').slice(0, 40);
		// Thêm 4 chữ số cuối của timestamp để tránh trùng lặp
		...
		if (ce || !created) {
			// Có thể trigger DB đã tạo row trước, thử refetch
	...
	private mapProfileRow(data: Record<string, unknown>): UserProfile {
		// User_Role(Role(RoleName)) → [{ Role: { RoleName: '...' } }]
