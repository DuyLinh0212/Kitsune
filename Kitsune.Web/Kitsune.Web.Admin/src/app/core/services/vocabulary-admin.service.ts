...
export class VocabularyAdminService {
  ...
  // ✅ VocabularyFolder (singular), count via Vocabularies relation
  getFolders(): Observable<VocabularyFolderDto[]> {
    return from(supabase.from('VocabularyFolder').select('Id, UserId, FolderName, Description, IsPublic, CreatedAt, Vocabularies(count)').order('FolderName')).pipe(
      map(({ data, error }) => {
  ...
  createFolder(dto: CreateFolderDto): Observable<VocabularyFolderDto> {
    return from(supabase.from('VocabularyFolder').insert({ FolderName: dto.folderName, Description: dto.description ?? null, IsPublic: dto.isPublic ?? false }).select('Id, UserId, FolderName, Description, IsPublic, CreatedAt').single()).pipe(
      map(({ data, error }) => {
        if (error) throw error;
  ...
  deleteFolder(id: number): Observable<void> {
    return from(supabase.from('VocabularyFolder').delete().eq('Id', id)).pipe(map(({ error }) => { if (error) throw error; }));
  }
  ...
  private async fetchVocabularies(query: VocabularyListQuery): Promise<PagedResult<VocabularyDto>> {
    ...
    let q = supabase.from('Vocabularies').select(VOCAB_SELECT, { count: 'exact' });
  ...
  private mapVocabularyRow(r: Record<string, unknown>): VocabularyDto {
    ...
    // ✅ KanjiComponents join returns objects with "Order" (PascalCase) and Kanji alias
    ...
    return {
      ...
      kanjiComponents: (components ?? [])
        ...
        .map((c) => ({ kanjiId: c.Kanji.Id, character: c.Kanji.Character, amHanViet: c.Kanji.AmHanViet, order: c.Order }))
    };
  }
}
