...
-- Users are learner nodes, KnowledgeSkills are capability nodes, and
-- LearningEvidence rows are append-only evidence edges produced by SRS/exams.
...
create table if not exists "LearningEvidence" (
  ...
  "CreatedAt" timestamptz not null default now(),
  check (
    ("SourceType" = 'SRS' and "SourceCardId" is not null and "SourceAttemptId" is null)
    or
    ("SourceType" = 'EXAM' and "SourceAttemptId" is not null and "SourceCardId" is null)
    or
    ("SourceType" = 'LEGACY' and "SourceAttemptId" is null and "SourceCardId" is null)
  ),
  unique ("UserId", "SessionKey", "SkillCode")
);

create index if not exists "idx_learning_evidence_user_time"
  on "LearningEvidence" ("UserId", "OccurredAt" desc);
create index if not exists "idx_learning_evidence_user_skill"
  on "LearningEvidence" ("UserId", "SkillCode", "OccurredAt" desc);
create index if not exists "idx_learning_evidence_attempt"
  on "LearningEvidence" ("SourceAttemptId") where "SourceAttemptId" is not null;
...
create policy "KnowledgeSkills_read" on "KnowledgeSkills"
  for select using (true);

drop policy if exists "LearningEvidence_owner_read" on "LearningEvidence";
create policy "LearningEvidence_owner_read" on "LearningEvidence"
  for select using (
...
create policy "LearningEvidence_owner_insert" on "LearningEvidence"
  for insert with check (
    "UserId" = (select "Id" from "Users" where "Email" = auth.jwt()->>'email')
  );

drop policy if exists "LearningEvidence_owner_delete" on "LearningEvidence";
create policy "LearningEvidence_owner_delete" on "LearningEvidence"
  for delete using (
    "UserId" = (select "Id" from "Users" where "Email" = auth.jwt()->>'email')
  );

create or replace view "LearningKnowledgeStats"
...
select
  ...
  skill."Label",
  skill."Category",
  count(*)::int4 as "Attempts",
  count(*) filter (where evidence."IsCorrect")::int4 as "Correct",
  round(
    count(*) filter (where evidence."IsCorrect")::numeric * 100
      / nullif(count(*), 0)
  )::int4 as "Score",
...
comment on table "KnowledgeSkills" is 'Canonical capability nodes used by the learner knowledge graph.';
comment on table "LearningEvidence" is 'Append-only evidence edges from a learner to a capability, produced by SRS or exams.';
comment on view "LearningKnowledgeStats" is 'RLS-aware per-user capability aggregation for Knowledge Graph rendering.';
